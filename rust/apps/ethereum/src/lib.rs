...
mod tests {
    ...
    use crate::eip712::eip712::{Eip712, TypedData as Eip712TypedData};
    use crate::{
        parse_eoa_code_tx, parse_fee_market_tx, parse_personal_message, parse_typed_data_message,
    ...
    #[test]
    fn test_parse_eoa_code_tx() {
        let raw = hex::decode("f84701808459682f008506fc23ac008252089431ba53ca350975007b27cf43acb4d9bc3db2641c8301e24980c0dbda0194123456789012345678901234567890123456789080808080").unwrap();
        let seed = hex::decode("5eb00bbddcf069084889a8ab9155568165f5c453ccb85e70811aaed6f6da5fc19a5ac40b389cd370d086206dec8aa6c43daea6690f20ad3d8d48b2d2ce9e38e4").unwrap();
        let path = "m/44'/60'/0'/0/0".to_string();
        let pubkey = get_public_key_by_seed(&seed, &path).unwrap();

        let result = parse_eoa_code_tx(&raw, Some(pubkey)).unwrap();
    }
